  ...
  try {
    ...
    if (!title || !videoUrl || !description || !thumbnail) {
      return next(
        new apiError(
          400,
          "title, videourl, description, and thumbnail is required"
        )
      );
    }
...
const watchVideo = asyncHandler(async (req, res, next) => {
  ...
  const relatedVideos = await Video.find({ _id: { $ne: videoId } })
    .limit(5)
    .select("title thumbnail owner views likes")
  ...
  const likeByUser = await Like.findOne({
    likeOn: videoId,
    likeBy: req.userId,
  });

  // Comments
  const comments = await Comment.find({ commentOn: videoId, isHidden: false })
  ...
  res
    ...
    .json(
      new apiResponse(
        ...
        "Video and related videos fetch successfully"
      )
    );
});

const likeOnVideo = asyncHandler(async (req, res, next) => {
  const { videoId } = req.params;
  const userId = req.user._id;

  const video = await Video.findById(videoId)
  if(!video){
    return next(new apiError(404, null, "video not found"))
  }

  const alreadyLiked = await video.likes.includes(userId)

  if(alreadyLiked){
    video.likes = await video.likes.filter(id => id.toString() !== userId.toString());
    await video.save();
...
const commentOnVideo = asyncHandler(async (req, res, next) => {
  ...
  await comment.populate("commentBy", "username avatar");
...
const myCommentsVideos = asyncHandler(async (req, res, next) => {
  try {
    const comments = await Comment.find({ commentBy: req.user })
      .select("commentOn")
    ...
    res
      .status(200)
      .json(
        new apiResponse(200, videos, "Fetched all videos you commented on.")
      );
  } catch (error) {
    next(error);
  }
...
const myLikesVideos = asyncHandler(async (req, res, next) => {
  try {
    const likes = await Like.find({ likeBy: req.user })
      .select("likeOn")
      .populate("likeOn", "title, thumbnail")
      .populate("likeBy", "username");

    if (!likes.length) {
      return res.status(404).json(new apiResponse(404, null, "No liked video"));
    }
    ...
    res
      ...
      .json(
        new apiResponse(
          200,
          deletedVideos,
          "All your videos are deleted successfull!"
        )
      );
  } catch (error) {
    next(error);
